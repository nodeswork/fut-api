...
module.exports = function(options){
  ...
  function loginForm(url) {
    defaultRequest.post(url, {
      ...
      bodyLines = __.filter(bodyLines, function(x) { return x.indexOf('redirectUri') >= 0 });

      __.each(bodyLines, function(l) {
        var match = re.exec(l);
        if (match) {
          redirectUri += match[1];
        }
      ...
      defaultRequest.get(redirectUri, function(error, response, body) {
        if(body.indexOf("<title>FIFA Football | FUT Web App | EA SPORTS</title>") > 0) return getNucleus();
  ...
  function getShards() {
    ...
    defaultRequest.get(urls.login.shards, function(error, response, body){
      if(error) return loginDetails.loginCb(error);
